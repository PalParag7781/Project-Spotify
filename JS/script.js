...
let url = new URL(element.href) // element.href is a string so it makes it like a object which has all its info.
let pathname = url.pathname 
let  songname =decodeURIComponent(url.pathname.split("/").pop().replace(/\.(mp3|m4a)$/i,"")); 
...
async function displayALBUMS(){
 ...
 let array = Array.from(anchors) 
  for (let index = 0; index < array.length; index++) {
    const e = array[index];
    console.log(`the e.href is ${e.href}`)
